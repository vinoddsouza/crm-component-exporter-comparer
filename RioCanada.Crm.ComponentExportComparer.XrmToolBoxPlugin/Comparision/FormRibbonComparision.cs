...
{
    public partial class FormRibbonComparision : Form
    {
        ...
        public FormRibbonComparision(Settings settings, string source, string target)
        {
            InitializeComponent();
        ...
        private void FormSecurityRoleComparision_Load(object sender, EventArgs e)
        {
            ...
            bgw.DoWork += (ss, ee) =>
            {
                List<IndexLineItem> sourceData = null;
                List<IndexLineItem> targetData = null;

                var s = Core.IndexLineItemGenerator.GenerateForRibbon(this.Source);
                var t = Core.IndexLineItemGenerator.GenerateForRibbon(this.Target);

                sourceData = s.Children;
                targetData = t.Children;

                this.TreeViewData = ComparisionView.GetComparisionData(sourceData, targetData);
            };
        ...
        private void RenderTreeViewData()
        {
            ...
            this.TreeViewData.ForEach(x =>
            {
                ...
                rows.Add(GetTreeViewRow(0, x, x.Name, (x.DefaultItem as Core.RibbonIndexLineItems.Element).Collapsed ? RightArrow : DownArrow, x.Status));

                if (x.Children != null && x.Children.Count > 0 && !(x.DefaultItem as Core.RibbonIndexLineItems.Element).Collapsed)
                {
                    x.Children.ForEach(y =>
                    {
                        if ((y.Status & this.comparisionStatusCheckboxFilter1.Status) == 0) return;
                        if ((y.DefaultItem as Core.RibbonIndexLineItems.Element).IsMetadata)
                        {
                        ...
                        else
                        {
                            ...
                            rows.Add(GetTreeViewRow(1, y, y.Name, (y.DefaultItem as Core.RibbonIndexLineItems.Element).Collapsed ? RightArrow : DownArrow, y.Status));
                        }

                        if (y.Children != null && y.Children.Count > 0 && !(y.DefaultItem as Core.RibbonIndexLineItems.Element).Collapsed)
                        {
                            y.Children.ForEach(z =>
                            {
                                if ((z.Status & this.comparisionStatusCheckboxFilter1.Status) == 0) return;
                                rows.Add(GetTreeViewRow(2, z, z.Name, null, z.Status));
                            });
                        }
                    });
                }
        ...
        private void HandleCollapseChange(DataGridViewRow row, bool flag)
        {
            ...
            var roleItem = item.DefaultItem as Core.RibbonIndexLineItems.Element;
        ...
        private void ChangeCollapseAllStatus(bool value)
        {
            this.TreeViewData.ForEach(x =>
            {
                (x.DefaultItem as Core.RibbonIndexLineItems.Element).Collapsed = value;
                ...
                if (x.Children != null && x.Children.Count > 0)
                {
                    x.Children.ForEach(y =>
                    {
                        ...
                        (y.DefaultItem as Core.RibbonIndexLineItems.Element).Collapsed = value;
                    });
                }
            });
        }
